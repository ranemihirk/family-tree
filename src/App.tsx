...
export default function App(): JSX.Element {
  return (
    <div className="App min-h-screen w-screen bg-slate-500">
...
const FamilyTree: React.FC<FamilyTreeProps> = ({ familyMembers, childrenCount }) => {
	// console.log('familyMembers: ', familyMembers, familyMembers.length);
	for(let current in familyMembers){
		console.log('current children: ', familyMembers[current], familyMembers[current].children.length);
      ...
      <ul className="list-none flex justify-evenly">
        {familyMembers.map((item, idx) => (
          ...
          >
            ...
            <FamilyTree familyMembers={item.children} childrenCount={item.children.length} />
          </li>
        ))}
      </ul>
    </div>
